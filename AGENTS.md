...
- `shows/nobody-moves/` — TikTok series production pipeline (still images → Ken Burns + TTS voices + synthesized score → 1080×1920 MP4). Independent of the skill; one folder per episode under `episodes/`. See `shows/nobody-moves/README.md`. Project skills `nobody-moves-write-episode`, `nobody-moves-produce-episode` and `nobody-moves-sound-design` (in `.agents/skills/`, symlinked from `.claude/skills/`) drive it; `pipeline/mixcheck.py` measures a built episode's mix. The `ai-tiktok-series` skill scaffolds a new show from it under `shows/<slug>/`. For episodes the pipeline builds, sound changes go through `nobody-moves-sound-design`, not `cinematic-sound-designer` (build_audio.py regenerates all episode audio).
