...
- **"Will it work for Reels and Shorts?"** The file does, but the layout had only been judged against TikTok. `pipeline/safezones.py` found text under the UI in every episode: the board's index card ends at y 1553, under every app's description, because the board's push-in carries it down. It finds text by rendering each moment with and without text and differencing the two, so nothing has to be listed by hand.
