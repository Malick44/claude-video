...
- **"Use my API key in `/Users/.../.env.local`"** points at the user's own computer, which a cloud session can't read. A key reaches the session as an environment variable set in the cloud environment's settings, never pasted into chat. Here the Higgsfield hosts were also denied by the network policy (403), so Episode 4 shipped with a derived still (`ray_night`) and prompts for the user to generate on their machine.
- **Build audio one episode at a time.** Three `build_audio.py` runs at once on a 4-core container ran for over 17 minutes without finishing; one at a time took 24–35 s each. Render frames in parallel afterwards.
...
- **"Will it work for Reels and Shorts?"** The file does, but the layout had only been judged against TikTok. `pipeline/safezones.py` found text under the UI in every episode. The board's index card ended at y 1553, under every app's description, because the board's push-in carried it down. It couldn't move up on the board without covering the bottom polaroids' labels, so the fix was the camera: the push-in now ends framed lower (center y 0.48 to 0.53), which lifts the card to y 1444. The bottom polaroids moved in (0.23/0.78 to 0.29/0.72) so their labels clear the buttons. The question cards (wrap 860 to 780) and a long end-card "NEXT" line (auto-fit) were pulled in from the right edge too. The check finds text by rendering each moment with and without text and differencing the two, so nothing has to be listed by hand.
