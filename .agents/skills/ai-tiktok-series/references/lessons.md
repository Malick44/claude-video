...
- **There is no ffprobe.** Use the `imageio_ffmpeg` binary: `FF=$(.venv/bin/python -c "import imageio_ffmpeg;print(imageio_ffmpeg.get_ffmpeg_exe())")`. Get specs from `$FF -i file` and loudness from `$FF -i file -af ebur128=peak=true -f null -`.
...
- **You can't listen, so measure.** Every mix decision was checked by numbers:
  - Mud: the score had 35.7% of its energy below 60 Hz. A highpass on the bed at 45 Hz and on the master at 30 Hz, less drone and cello, and added violins brought it to 20.2%.
  - Phone speakers: the impact had only 1% of its energy above 250 Hz, so phones barely played it. A slam layer, a noise boom and saturated harmonics raised it to 21.4%.
  - Final Episode 2 figures: −14.0 LUFS, true peak −1.5 dBFS, dialogue a median of about 20 dB over the music (9.6 dB at worst, on a line deliberately under wind), and a "music out" shot that measured fully silent.
  - `pipeline/mixcheck.py episodes/<ep>`, where the pipeline has it, runs all of these checks in one command. Build with `pipeline/build_audio.py episodes/<ep> --stems` first: without stems it runs only the soundtrack-level checks and skips dialogue over music, music-out and the hits.
- **Dialogue buried?** Move the hit or trim `LEVELS`. Don't turn the voices up; they are RMS-matched to −17 dB per line.
