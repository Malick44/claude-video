...
- **Captions carry the comedy.** Most people watch muted, so every joke has to read as text. Keep lines short, one idea each, and put the punchline last. Use a `P(...)` pause before a punchline; the silence is the joke's timing. A caption shows at most two lines of about 20 characters, so a line over about 40 characters is split into chunks, at a full stop or comma when it has one; punctuate where a split should fall.
