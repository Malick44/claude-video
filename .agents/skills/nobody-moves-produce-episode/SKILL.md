...
- **Every app's UI:** `.venv/bin/python pipeline/safezones.py episodes/<ep>` (about 45 s, no render needed) flags text under the buttons, top bar or description of TikTok, Reels or Shorts and draws `build/review/zones.png`. A push-in (`kb` zoom above 1) carries text near the edges outward as the shot goes on: frame it so the text ends inside, or move the element. The board's push-in ends framed low, `(0.5, 0.53, 1.18)`, so the index card stays above every description, and its bottom row of polaroids sits at 0.29 and 0.72 so their labels clear the buttons. Keep both when you update the board.
